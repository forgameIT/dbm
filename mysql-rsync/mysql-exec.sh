...
function_update()
{
    if [ -f "${FILE_PATH}/update.sql" ]; then
        ...
        if [ -z `cat ${FILE_PATH}/.update.history.log|grep $SQLHASH` ]; then
            mysql -h"${CONFIG_MYSQL_HOST}" -u"${CONFIG_MYSQL_USER}" -p"${CONFIG_MYSQL_PASSWORD}" -P"${CONFIG_MYSQL_PORT}" -e "CREATE DATABASE IF NOT EXISTS ${CONFIG_MYSQL_DATABASE} DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci"
            mysql -h"${CONFIG_MYSQL_HOST}" -u"${CONFIG_MYSQL_USER}" -p"${CONFIG_MYSQL_PASSWORD}" -P"${CONFIG_MYSQL_PORT}" -D${CONFIG_MYSQL_DATABASE} < ${FILE_PATH}/update.sql
...
function_backup()
{
    ...
    mysqldump -h"${CONFIG_MYSQL_HOST}" -u"${CONFIG_MYSQL_USER}" -p"${CONFIG_MYSQL_PASSWORD}" -P"${CONFIG_MYSQL_PORT}" --skip-lock-tables --add-drop-database --add-drop-table --databases ${CONFIG_MYSQL_DATABASE} > ${FILE_PATH}/latest.sql
